...
add_task(async function test_helpers_getFxAStatus_has_sync_keys() {
  ...
  let { signedInUser } = await helpers.getFxaStatus("sync", mockSendingContext);
  Assert.strictEqual(signedInUser.hasSyncKeys, true, "reports the sync keys");
  deepEqual(requestedScopes, [SCOPE_APP_SYNC], "checks the sync scope");

  hasSyncKeys = false;
  ({ signedInUser } = await helpers.getFxaStatus("sync", mockSendingContext));
  Assert.strictEqual(
    signedInUser.hasSyncKeys,
    false,
